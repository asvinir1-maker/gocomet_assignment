...
export default function Landing() {
  ...
  useEffect(() => {
    let cancelled = false;
    setListLoading(true);
    axios.get(`${API}/shipments`, { params: statusFilter !== "all" ? { status: statusFilter } : {} })
  ...
  const handleTrack = async (id) => {
    ...
    setTracked(null);
    try {
      const r = await axios.get(`${API}/shipments/${encodeURIComponent(id)}`);
      setTracked(r.data);
      ...
      const msg = e.response?.data?.detail || "Shipment not found";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div data-testid="landing-page">
      <HeroSearch onTrack={handleTrack} loading={loading} />
        ...
        <section id="tracked-result" className="px-6 md:px-12 py-12 border-b border-neutral-200 bg-white" data-testid="tracked-result-section">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
              ...
              <StatusBadge status={tracked.status} className="text-xs" />
            </div>
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                <button
                  key={t.k}
                  onClick={() => {
                    if (t.k === "all") setParams({});
                    else setParams({ status: t.k });
                  }}
      ...
      <footer className="px-6 md:px-12 py-8 border-t border-neutral-200 bg-[#FAFAFA]">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="font-mono text-[11px] text-neutral-500">v1.0 · {new Date().getFullYear()}</div>
        </div>
      </footer>
    </div>
  );
